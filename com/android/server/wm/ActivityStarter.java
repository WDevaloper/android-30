...
class ActivityStarter {
    ...
    private int executeRequest(Request request) {
        ...
        // 检查启动Actiivty 的权限  如：后台不能启动Activity等等
        ...
        // 合并两个选项包，而 realCallerOptions 优先。
        ...
        //在完成大部分初步检查并确认调用者拥有执行此操作所需的权限后启动Activity。如果启动不成功，这里还确保删除启动活动。
        mLastStartActivityResult = startActivityUnchecked(
                r,
                sourceRecord,
                voiceSession,
                request.voiceInteractor,
                startFlags,
                true /* doResume */,
                checkedOptions,
                inTask,
    ...
    /**
     * 启动Activity并确定Activity是否应该添加到现有任务栈的顶部或向现有Activity传递新Intent。还将Activity任务栈操作到请求的或有效的堆栈显示上。
     * <p>
     * 就是说：确认 是否新建Activity，还是直接传递新Intent   并且将Activity的任务栈
     * <p>
    ...
    int startActivityInner(final ActivityRecord r, ActivityRecord sourceRecord,
        ...
        // 即：根据我们的launchMode 是否重新创建新的Activity     onNewIntent
        ...
        mTargetStack.startActivityLocked(
                mStartActivity,
                topStack.getTopNonFinishingActivity(),
                newTask,
                mKeepCurTransition,
                mOptions
        );
        ...
        if (mDoResume) {
            ...
            if (!mTargetStack.isTopActivityFocusable()
                ...
                mRootWindowContainer
                        .resumeFocusedStacksTopActivities(mTargetStack, mStartActivity, mOptions);
            }
        }
